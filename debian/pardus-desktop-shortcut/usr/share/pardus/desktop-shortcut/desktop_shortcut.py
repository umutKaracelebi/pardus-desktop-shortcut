...
class DesktopShortcut:
    ...
    def copy_to_desktop(self, source_path, overwrite=False):
        ...
        if not os.path.isfile(source_path):
            return False, _("{} not found.").format(source_path)

        if not source_path.endswith(".desktop"):
            return False, _("Only .desktop files are supported.")

        filename = os.path.basename(source_path)
        dest_path = os.path.join(self.desktop_dir, filename)

        if os.path.exists(dest_path) and not overwrite:
            return False, _("{} is already on the desktop.").format(filename)
        ...
        try:
            ...
            return False, _("Copy error: {}").format(str(e))

    # ------------------------------------------------------------------
    # Herhangi bir dosya veya klasörü masaüstüne kısayol (symlink) yap
    # ------------------------------------------------------------------
    def link_to_desktop(self, source_path, overwrite=False):
        """
        Bir dosya veya klasör için masaüstünde sembolik bağ (kısayol) oluştur.
        
        Args:
            source_path: Kaynak dosya/klasörün tam yolu
            overwrite: Mevcut dosyanın üzerine yaz
        ...
        try:
            ...
            return False, _("Shortcut creation error: {}").format(str(e))
    ...
    def create_desktop_entry(self, name, exec_path, icon="", comment="",
        ...
        if not name or not name.strip():
            return False, _("Application name cannot be empty.")

        if not exec_path or not exec_path.strip():
            return False, _("Execution command cannot be empty.")
        ...
        try:
            ...
            return False, _("Creation error: {}").format(str(e))
    ...
    def remove_from_desktop(self, desktop_filename):
        ...
        if not os.path.exists(dest_path):
            return False, _("{} was not found on the desktop.").format(desktop_filename)

        try:
            os.remove(dest_path)
            return True, _("{} removed from desktop.").format(desktop_filename)
        except OSError as e:
            return False, _("Removal error: {}").format(str(e))
